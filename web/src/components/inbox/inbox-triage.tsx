...
export function InboxTriage({ inbox }: { inbox: InboxJob[] }) {
  ...
  const skip = (job: InboxJob) => {
    ...
    setUndo({ label: t("inbox.skipped", { c: job.company }), fn: () => setHidden((h) => h.filter((u) => u !== job.url)) });
  };
    ...
    <div className={cn("mx-auto mt-4 max-w-3xl", shortlist.length > 0 && "pb-28 sm:pb-24")}>
      ...
      <div className="mt-4 flex items-baseline justify-between gap-3">
        <p className="text-sm font-medium text-foreground">
          {capped ? t("inbox.freshWorthALook") : anyFacet ? t("inbox.matches", { n: filtered.length }) : t("inbox.allRoles")}
        </p>
        {hiddenCount > 0 && (
      ...
      {visible.length > 0 ? (
        ...
        <div className="mt-3 rounded-2xl border border-dashed border-border bg-surface/30 px-6 py-10 text-center">
          ...
          <p className="mx-auto mt-1 max-w-sm text-sm text-muted">{t("inbox.noMatchesInbox")}</p>
        </div>
      )}
      ...
      {capped && ordered.length > BATCH && (
        ...
        >
          {t("inbox.seeAllInbox", { n: ordered.length })}
        </button>
      )}

      {/* empty-shortlist guidance (only once there's nothing saved) */}
      {shortlist.length === 0 && (
        <p className="mt-4 text-center text-xs text-faint">{t("inbox.saveRolesHint")}</p>
      )}
